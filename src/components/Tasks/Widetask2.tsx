...
interface WideTask2Props {
  task: Task;
  onClose: () => void;
  onUpdate: (updatedTask: Task) => void;
}

export default function WideTask2({ task, onClose, onUpdate }: WideTask2Props) {
  ...
  const toggleItem = (id: number) => setChecklist(prev => prev.map(i => i.id === id ? { ...i, done: !i.done } : i));
  const deleteItem = (id: number) => setChecklist(prev => prev.filter(i => i.id !== id));
  const addItem = () => {
    if (newItem.trim()) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-md" onClick={onClose}>
      ...
      >
        ...
        <h2 className="text-center text-xl font-bold mb-4 pr-8 text-[#5E417B]">{task.title}</h2>
        ...
        <div className="flex justify-between items-center px-4 mb-5">
          ...
          <div className="bg-[#A078C9] text-white rounded-full px-5 py-2 font-bold text-sm shadow-md">
